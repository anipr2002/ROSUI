      ...
      {isOpen && (
        <>
          ...
          <motion.div
            ...
            className={cn(
              "fixed z-50 overflow-hidden border border-zinc-950/10 bg-white shadow-lg outline-none dark:border-zinc-50/10 dark:bg-zinc-800 rounded-md",
              className
            )}
            style={{
              borderRadius: 12,
...
function FloatingPanelTitle({ children }: FloatingPanelTitleProps) {
  ...
  return (
    <motion.div
      layoutId={`floating-panel-label-container-${uniqueId}`}
      className="px-4 mt-2 bg-white dark:bg-zinc-800 text-center"
    >
      <motion.div
        layoutId={`floating-panel-label-${uniqueId}`}
